...
struct CudaRenderBufferView {
	//* 어떤 값을 여기에 넣기 위해서는 순서가 중요하다.
	//* std::pair<uint32_t, uint32_t>* coords_buffer = nullptr;
	Triple* position_buffer = nullptr;
	Triple* direction_buffer = nullptr;
	Triple* rgb_buffer = nullptr;
	float* density_buffer = nullptr;
	float* dist_buffer = nullptr;
	...
	uint32_t spp = 0;
	uint32_t desired_value_for_idx = 5;
	uint32_t desired_value = 20;
...
public:
	...
	uint32_t desired_value_for_idx() const{
		return m_desired_value_for_idx;
	}

	uint32_t desired_value() const {
	...
	// ! ---------------------- !//
	// std::pair<uint32_t, uint32_t>* coords_buffer() const {
	// 	return m_coords_buffer.data();
	// }

	Triple* position_buffer() const{
		return m_position_buffer.data();
	}

	Triple* direction_buffer() const{
		return m_direction_buffer.data();
	}

	Triple* rgb_buffer() const{
		return m_rgb_buffer.data();
	}

	float* dist_buffer() const{
		return m_dist_buffer.data();
	}

	float* density_buffer() const{
		return m_density_buffer.data();
	}

	uint32_t* indices_buffer() const {
		return m_indices_buffer.data();
	}
...
private:
	uint32_t m_spp = 0;
	uint32_t m_desired_value = 20;
	uint32_t m_desired_value_for_idx = 5;
	...
	//* 수정 부분
	//! ------------------------------------- !//
	//* tcnn::GPUMemory<std::pair<uint32_t, uint32_t>> m_coords_buffer;
	tcnn::GPUMemory<Triple> m_position_buffer;  //vec3는 나중에
	tcnn::GPUMemory<Triple> m_direction_buffer;
	tcnn::GPUMemory<Triple> m_rgb_buffer;
	tcnn::GPUMemory<float> m_dist_buffer;
	tcnn::GPUMemory<float> m_density_buffer;
	tcnn::GPUMemory<uint32_t> m_indices_buffer;
