...
public:
	...
	void prepare_next_camera_path_frame(bool& flag);
	void imgui();
	...
	std::vector<View> m_views; //* CudaRenderBuffer가 들어있는 객체
	ivec2 m_n_views = {1, 1};
	...
	struct ImGuiVars {
		...
		char ray_path[MAX_PATH_LEN] = "./points_data/lego/points_001.bin";
		char files_path[MAX_PATH_LEN] = "./points_data/lego/";
	} m_imgui;
	...
	std::pair<uint32_t, uint32_t>* m_coords_buffer = nullptr;	// n_pixels
	Triple* m_position_buffer = nullptr; // n_pixels * spp
	Triple* m_direction_buffer = nullptr;
	Triple* m_rgb_buffer = nullptr;
	float*  m_dist_buffer = nullptr;
	float*  m_density_buffer = nullptr;
	uint32_t* m_indices_buffer = nullptr;
	// NerfPayload* m_payload_buffer = nullptr;

	bool m_button = false;
	bool m_save_json_file = false;
	bool m_save_files = false;
	vec4* m_rgba[2];
	uint32_t m_cnt = 0;

	std::pair<uint32_t, uint32_t>* m_desired_coords_buffer;
