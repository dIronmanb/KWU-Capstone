...
void Testbed::imgui() {
	...
	if (ImGui::Begin("Camera path", 0, ImGuiWindowFlags_NoScrollbar)) {
		...
		if (!m_camera_path.keyframes.empty() && ImGui::CollapsingHeader("Export video", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			// button flag
			if (imgui_colored_button(m_button ? "Switch False" : "Switch True", 0.2)) {
				m_button = !m_button;
			...
			// save json file
			if(imgui_colored_button(m_save_json_file ? "Stop Saving" : "Start Saving to a binary file", 0.8)){
				ImGui::InputText("File Path to Save", m_imgui.ray_path, sizeof(m_imgui.ray_path));
				m_camera_path.render_settings.file_path_to_save = m_imgui.ray_path;
				m_save_json_file = !m_save_json_file;
			}

			// save json file dump
			if(imgui_colored_button(m_save_files ? "Stop Saving" : "Start Saving to binary files", 0.9)){
				ImGui::InputText("File Path to Save", m_imgui.files_path, sizeof(m_imgui.files_path));
				m_camera_path.render_settings.file_path_to_save = m_imgui.files_path;
	...
	if (m_testbed_mode == ETestbedMode::Nerf || m_testbed_mode == ETestbedMode::Sdf) {
		if (ImGui::CollapsingHeader("Export mesh / volume / slices")) {
			...
			if (imgui_colored_button("Mesh it!", 0.4f)) {
				//* Mesh it!를 시작하면 m_mesh 안에 값들이 저장됨
...
bool Testbed::keyboard_event() {
	...
	if (ImGui::IsKeyPressed('[')) {
		if (shift) { // 이전 이미지가 없다 = shift가 True다
			first_training_view();
		} else {
			previous_training_view();
		}
...
void Testbed::prepare_next_camera_path_frame(bool& flag) {
	if (!flag) {
		return;
	}
...
bool Testbed::frame() {
	...
	if (m_camera_path.rendering) {
		prepare_next_camera_path_frame(m_camera_path.rendering);
		skip_rendering = false;
	}

	// if(m_button){
	// 	prepare_next_camera_path_frame(m_button);
	// 	skip_rendering = false;
	// }
	...
	switch (m_testbed_mode) {
		case ETestbedMode::Nerf:
			if (!m_render_ground_truth || m_ground_truth_alpha < 1.0f) {
				...
				//! device.render_buffer_view(): testbed.h 1061 수정
				...
				uint32_t spp = device.render_buffer_view().spp + 1;
				if(spp > 70){
					//! spp의 상한선 정해주기
					reset_accumulation(true);
				}
...
ScopeGuard Testbed::use_device(cudaStream_t stream, CudaRenderBuffer& render_buffer, Testbed::CudaDevice& device) {
	...
	//* 다양한 타입에 대해서도 가능해야 하는데, 얘는 그렇지를 않아...
	//? 2개면 넣으면 된다고? 3개 이상으로는 확장할 수가 없어???
	//auto scratch = allocate_workspace_and_distribute<vec2, vec4, float>(device.stream(), &alloc, n_pixels, n_pixels, n_pixels);
	//! spp가 잘 들어 갔는지를 확인해야 한다.
	uint32_t spp = render_buffer.spp();
	auto scratch = allocate_workspace_and_distribute<
		...
		device.stream(),
		&alloc,
		n_pixels*(spp+1), //* position buffer
		n_pixels*(spp+1), //* direction buffer
		n_pixels*(spp+1), //* rgb buffer
		n_pixels*(spp+1), //* dist buffer
		n_pixels*(spp+1), //* density buffer
		n_pixels*(spp+1), //* indices buffer
		n_pixels, //* frame buffer
		n_pixels  //* depth buffer
		);
	...
	device.set_render_buffer_view({
		...
		render_buffer.spp(),
		render_buffer.desired_value_for_idx(), //* index를 어디서 초기화 할까?
		render_buffer.desired_value(),
	...
	return ScopeGuard{make_copyable_function([&render_buffer, &device, guard=std::move(guard), alloc=std::move(alloc), active_device, stream]() {
		...
		// 	render_buffer.coords_buffer(),
		// 	active_device,
		// 	device.render_buffer_view().coords_buffer,
		// 	device.id(),
		// 	compMul(render_buffer.in_resolution()) * sizeof(std::pair<uint32_t, uint32_t>),
		// 	device.stream()));

		CUDA_CHECK_THROW(cudaMemcpyPeerAsync(
			render_buffer.position_buffer(),
			active_device,
			device.render_buffer_view().position_buffer,
			device.id(),
			(device.render_buffer_view().spp+1) * compMul(render_buffer.in_resolution()) * sizeof(Triple),
			device.stream()));

		CUDA_CHECK_THROW(cudaMemcpyPeerAsync(
			render_buffer.direction_buffer(),
			active_device,
			device.render_buffer_view().direction_buffer,
			device.id(),
			(device.render_buffer_view().spp+1)  * compMul(render_buffer.in_resolution()) * sizeof(Triple),
			device.stream()));
		...
		CUDA_CHECK_THROW(cudaMemcpyPeerAsync(
			...
			active_device,
			device.render_buffer_view().rgb_buffer,
			device.id(),
			(device.render_buffer_view().spp+1) * compMul(render_buffer.in_resolution()) * sizeof(Triple),
			device.stream()));
		...
		CUDA_CHECK_THROW(cudaMemcpyPeerAsync(
			...
			active_device,
			device.render_buffer_view().dist_buffer,
			device.id(),
			compMul(render_buffer.in_resolution()) * sizeof(float), device.stream()));

		CUDA_CHECK_THROW(cudaMemcpyPeerAsync(
			render_buffer.density_buffer(),
			active_device,
			device.render_buffer_view().density_buffer,
			device.id(),
			(device.render_buffer_view().spp+1)  * compMul(render_buffer.in_resolution()) * sizeof(float),
			device.stream()));
		...
		CUDA_CHECK_THROW(cudaMemcpyPeerAsync(
			...
			active_device,
			device.render_buffer_view().indices_buffer,
			device.id(),
			compMul(render_buffer.in_resolution()) * sizeof(uint32_t), device.stream()));
		...
		CUDA_CHECK_THROW(cudaMemcpyPeerAsync(
			...
			device.id(),
			compMul(render_buffer.in_resolution()) * sizeof(vec4), device.stream()));
