...
__device__ void advance_pos_nerf(
	...
	if (t >= MAX_DEPTH()) {
		//* t가 최대 깊이보다 크면 payload의 상태는 DEAD로
		payload.alive = false;
	} else {
		//* 그게 아니면 t로
		payload.t = t; //* sample_index에 따라서 거리가 달라진다!
	}
}
...
__global__ void generate_next_nerf_network_inputs(
	...
	//* 내가 보기엔, GPU에서 돌릴 수 있는 크기 같은데
	for (uint32_t j = 0; j < n_steps; ++j) {
		t = if_unoccupied_advance_to_next_occupied_voxel(t, cone_angle, {origin, dir}, idir, density_grid, min_mip, max_mip, render_aabb, render_aabb_to_local);
		if (t >= MAX_DEPTH()) {
			payload.n_steps = j; // t가 MAX_DEPTH()에 도달하면 payload.n_steps를 지정
			return;
		}

		float dt = calc_dt(t, cone_angle);
		//* 여기서 network의 input를 정해줌
...
__global__ void composite_kernel_nerf(
	...
	Triple* __restrict__ position_buffer,
	Triple* __restrict__ direction_buffer,
	Triple* __restrict__ rgb_buffer,
	float* __restrict__ dist_buffer,
	float* __restrict__ density_buffer,
	uint32_t* __restrict__ indices_buffer,
	...
	if (i >= n_elements) return;	//! n_elements = resolution.x * resolution.y;

	NerfPayload& payload = payloads[i];

	//! 나중에 다시 풀기
	//! payload 수정
	indices_buffer[i + idx_offset1] = payload.idx;
	...
	for (; j < actual_n_steps; ++j) {
		...
		vec3 warped_dir = input->dir.d; // ! ------------------ !//
		vec3 dir = unwarp_direction(warped_dir); // ! ------------------ !//

		float T = 1.f - local_rgba.a; // previous exp(-density * dt)
		float dt = unwarp_dt(input->dt);
		float density = network_to_density(float(local_network_output[3]), density_activation);
		float alpha = 1.f - __expf(-density * dt); // current alpha
		if (show_accel >= 0) {
			alpha = 1.f;
		}
		float weight = alpha * T; // current alpha * exp(과거의 총합인 T)

		vec3 rgb = network_to_rgb_vec(local_network_output, rgb_activation);

		/*
			1. T = 1 - i번째 픽셀의 rgba의 a (즉, 갱신되는 값)
			2. dt = pos[:-1] - pos[1:]
			3.
		*/

		position_buffer[idx_offset2 + (i + j * n_elements)].first = pos.x;
		position_buffer[idx_offset2 + (i + j * n_elements)].second = pos.y;
		position_buffer[idx_offset2 + (i + j * n_elements)].third = pos.z;

		direction_buffer[idx_offset2 + (i + j * n_elements)].first = dir.x;
		direction_buffer[idx_offset2 + (i + j * n_elements)].second = dir.y;
		direction_buffer[idx_offset2 + (i + j * n_elements)].third = dir.z;

		rgb_buffer[idx_offset2 + (i + j * n_elements)].first = rgb.r;
		rgb_buffer[idx_offset2 + (i + j * n_elements)].second = rgb.g;
		rgb_buffer[idx_offset2 + (i + j * n_elements)].third = rgb.b;
		density_buffer[idx_offset2 + (i + j * n_elements)] = density;

		dist_buffer[idx_offset2 + (i + j * n_elements)] = dt;


		// (res.x * res.y - 1) * (j~) * [i]
		// position_buffer[i + k * n_elements].first = pos.x;
		// position_buffer[i + k * n_elements].second = pos.y;
		// position_buffer[i + k * n_elements].third = pos.z;

		// direction_buffer[i + k * n_elements].first = dir.x;
		// direction_buffer[i + k * n_elements].second = dir.y;
		// direction_buffer[i + k * n_elements].third = dir.z;

		// rgb_buffer[i + k * n_elements].first = rgb.r;
		// rgb_buffer[i + k * n_elements].second = rgb.g;
		// rgb_buffer[i + k * n_elements].third = rgb.b;
		// density_buffer[i + k * n_elements] = density;

		// indices_buffer[i + k * n_elements] = payload.idx;

		// position_buffer[i*spp + j].first = pos.x;
		// position_buffer[i*spp + j].second = pos.y;
		// position_buffer[i*spp + j].third = pos.z;

		// direction_buffer[i*spp + j].first = dir.x;
		// direction_buffer[i*spp + j].second = dir.y;
		// direction_buffer[i*spp + j].third = dir.z;

		// rgb_buffer[i*spp + j].first = rgb.r;
		// rgb_buffer[i*spp + j].second = rgb.g;
		// rgb_buffer[i*spp + j].third = rgb.b;
		// density_buffer[i*spp + j] = density;
	...
	// j가 반복해야할 횟수보다 적다면 (batch 개수만큼이 아니라면)
	// 마지막 step이 된다.
	if (j < n_steps) {
		payload.alive = false;
...
__global__ void compact_kernel_nerf(
	const uint32_t n_elements, //* n_alive (살아있는 광선으 개수)
	...
	uint32_t* counter, //* m_alive_counter
	uint32_t* finalCounter //* m_hit_counter
) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
	...
	//* rays_tmp.payload[i]가 살아있다면
	//* 하나씩 옮기기
	if (src_payload.alive) {
		uint32_t idx = atomicAdd(counter, 1); //* 증가 연산 진행
		//* idx는 이전 값
		//* i는 이후 값
	...
	//* rays_tmp.rgba[i].a가 0.001f보다는 크다면 (유효하지 않은 광선이지만 밀도가 있는 광선)
	//? (그래도 밀도가 있다면..?)
	else if (src_rgba[i].a > 0.001f) {
		uint32_t idx = atomicAdd(finalCounter, 1); //* 증가 연산 진행
...
__global__ void init_rays_with_payload_kernel_nerf(
	uint32_t sample_index,	//* spp
	NerfPayload* __restrict__ payloads, //* payload array
	ivec2 resolution,
	vec2 focal_length,
	...
	//! 왜 이거만 진행하면 안될까? vec2-> printf는 CPU에서, BUT 해당 연산은 GPU에서 연산되기 때문에 //
	//! 이를 따로 처리하지 못하고 같이 연산하기 때문이다. //
	// vec2 tmp(x,y);
	//* coords는 이제 안 씀
	//* coords_buffer[idx].first = x;
	//* coords_buffer[idx].second = y;
	// printf("%d: (%d, %d)\n", idx, coords_buffer[idx].first, coords_buffer[idx].second);
	...
	NerfPayload& payload = payloads[idx]; //* idx번째의 payload
	...
	if (plane_z < 0) {
		...
		payload.t = -plane_z*n;
		payload.idx = idx;
		payload.n_steps = 0; //* z평면이 0보다 작으면
		payload.alive = false;
		depth_buffer[idx] = -plane_z; //* depth_buffer에는 음수가 채워짐
		return;
	}
	...
	payload.t = t;
	payload.idx = idx;
	payload.n_steps = 0; //? 여기서도 n_steps라고 적혀 있는데?
	payload.alive = true;
	//?????
}
...
void Testbed::NerfTracer::init_rays_from_camera(
	...
	init_rays_with_payload_kernel_nerf<<<blocks, threads, 0, stream>>>(
		sample_index, //* spp
		m_rays[0].payload, //* payload array
		resolution,
		focal_length,
		camera_matrix0,
		camera_matrix1,
	...
	//? 여기서도 spp만큼의 ray는 뽑아내지는 않고
	//? 다만, spp를 통하여 payload.alive가 false가 되기 직전까지만 뽑는데... (t >= MAX_DEPTH() 직전까지)
	...
	// for(uint32_t i=0 ; i < m_n_rays_initialized; i++)
	// 	printf("(%d, %d)\n", coords_buffer[i].first, coords_buffer[i].second);
}
...
uint32_t Testbed::NerfTracer::trace(
	...
	float* __restrict__ density_buffer,
	uint32_t* __restrict__ indices_buffer,
	std::vector<uint32_t>& n_alive_v,
	std::vector<uint32_t>& n_steps_between_compaction_v,
	uint32_t& cnt
) {
	...
	uint32_t n_alive = m_n_rays_initialized; //TODO resolution.x * resolution.y;
	...
	//! 내가 설정한 변수들
	// uint32_t cnt = 0;
	uint32_t idx_offset1 = 0;
	uint32_t idx_offest2 = 0;

	//?? 왜 i < MARCH_ITER만큼 계속 반복할까....
	//* 한 번에 volume rendering이 되지 않아서 루프를 주기적으로 돌아
	//* 조금씩 갱신되게 만든다.
	while (i < MARCH_ITER) {

		//* m_rays를 가지고 어떠한 조작을 반복적으로 수행한다.
		//* 현재 rays_current   (1, 0, 1, 0, ...)
		RaysNerfSoa& rays_current = m_rays[(double_buffer_index + 1) % 2];
		//* 이전의 rays_current  (0, 1, 0, 1, ...)
		RaysNerfSoa& rays_tmp = m_rays[double_buffer_index % 2];
		//* 아래 연산으로 인하여 rays_current, rays_tmp는 계속 변하게 된다.
		++double_buffer_index;

		// Compact rays that did not diverge yet
		{
			//* 아직 발산하지 않은 광선을 압축: 더 이상 필요하지 않은 광선을 제거하고 필요한 광선만 유지
			...
			linear_kernel(compact_kernel_nerf, 0, stream,
				...
				//* 매번 m_alive_counter를 새롭게 갱신하여 추적할 광선만 사용
			);
		...
		// Want a large number of queries to saturate the GPU and to ensure compaction doesn't happen too frequently.
		...
		linear_kernel(generate_next_nerf_network_inputs, 0, stream,
			...
			focal_length,
			camera_matrix[2],
			rays_current.payload, //* rays_current.payload를 주기적으로 줌
			input_data,
			n_steps_between_compaction, // n_steps_between_compaction....
			grid,
			(show_accel>=0) ? show_accel : 0,
			max_mip,
			cone_angle_constant,
			extra_dims_gpu
		); //* 여기서 payload.t를 가지고 n_steps를 갱신
		...
		linear_kernel(composite_kernel_nerf, 0, stream,
			...
			focal_length,
			depth_scale,
			//* rays_current의 rgba, depth, payload는 변함
			rays_current.rgba,
			rays_current.depth,
			rays_current.payload,
			//*
			input_data, // i = threadIdx.x + blockIdx.x * blockDim.x;
			m_network_output,
		...
		idx_offset1 += n_alive;
		idx_offest2 += n_steps_between_compaction * n_alive;


		//! n_alive를 하나씩 저장
		n_alive_v.push_back(idx_offset1);
		//! n_steps_between_compaction를 하나씩 저장
		n_steps_between_compaction_v.push_back(idx_offest2);
		//! cnt 증가
		++cnt;
...
void Testbed::render_nerf(
	...
	tracer.init_rays_from_camera(
		//* 여기서 spp에 의해 payload의 n_steps가 무슨 역할을 하는지를 봐야 한다.
		render_buffer.spp,
		...
		render_mode,
		stream
	); //* m_rays[0]에 payload 정보가 저장되었다.
	   //* m_rays[0]의 payload는 spp에 따라서 payload.t가 달라진다.

	uint32_t n_hit;
	//! n_alive_v, n_steps_between_compaction_v 추가
	std::vector<uint32_t> n_alive_v(1, 0);
	std::vector<uint32_t> n_steps_between_compaction_v(1, 0);
	uint32_t cnt=0;
	if (m_render_mode == ERenderMode::Slice) {
		...
		n_hit = tracer.trace(
			...
			extra_dims_gpu,
			stream,
			render_buffer.position_buffer,
			render_buffer.direction_buffer,
			render_buffer.rgb_buffer,
			render_buffer.dist_buffer,
			render_buffer.density_buffer,
			render_buffer.indices_buffer,
			n_alive_v,
			n_steps_between_compaction_v,
			cnt
		);

		if(m_button && (m_camera_path.render_settings.spp == render_buffer.spp + 1))
		{
			//! add from cam -> switch true -> UI에서 spp 값 바꾸기
			uint32_t spp = render_buffer.spp+ 1;
			ivec2 resolution = render_buffer.resolution;
			size_t n_pixels = resolution.x * resolution.y;
			uint32_t desired_value = render_buffer.desired_value;
			uint32_t desired_value_for_idx = render_buffer.desired_value_for_idx;

			tlog::info() << "Memory Setting! m_coords_buffer:" << m_coords_buffer <<
			"\nspp: " << spp <<
			"\ndesired value: " << desired_value <<
			"\ndesired value for idx: " << desired_value_for_idx <<
			"\nn_pixels: " << n_pixels << "(" << resolution.x <<  "," << resolution.y << ")" <<
			"\nbutton: " << m_button <<
			"\nrendering: " << m_camera_path.rendering;

			if(m_save_files)
			{
				if(m_cnt == 0) first_training_view();

				{
					tlog::info() << "Start saving files...(" << m_cnt << "th file)";
					std::string filename = m_camera_path.render_settings.file_path_to_save + "points_" + std::to_string(m_cnt) + ".bin";
					std::ofstream ofs(filename.c_str(), std::ios::binary);

					//! 어떻게 (0,0)으로 초기화 할까? -> 이미 (0,0)으로 초기화 되어 있다.
					//* m_coords_buffer = new std::pair<uint32_t, uint32_t>[n_pixels];
					m_position_buffer = new Triple[n_pixels * desired_value];
					m_direction_buffer = new Triple[n_pixels * desired_value];
					m_rgb_buffer = new Triple[n_pixels * desired_value];
					m_dist_buffer = new float[n_pixels * desired_value];
					m_density_buffer = new float[n_pixels * desired_value];
					m_indices_buffer = new uint32_t[n_pixels * desired_value_for_idx];

					//! copy results from device to host ! (GPU -> CPU)//
					//* CUDA_CHECK_THROW(cudaMemcpy(m_coords_buffer, render_buffer.coords_buffer, n_pixels * sizeof(std::pair<uint32_t, uint32_t>), cudaMemcpyDeviceToHost));
					CUDA_CHECK_THROW(cudaMemcpy(m_position_buffer, render_buffer.position_buffer, n_pixels * desired_value * sizeof(Triple), cudaMemcpyDeviceToHost));
					CUDA_CHECK_THROW(cudaMemcpy(m_direction_buffer, render_buffer.direction_buffer, n_pixels * desired_value * sizeof(Triple), cudaMemcpyDeviceToHost));
					CUDA_CHECK_THROW(cudaMemcpy(m_rgb_buffer, render_buffer.rgb_buffer, n_pixels * desired_value * sizeof(Triple), cudaMemcpyDeviceToHost));
					CUDA_CHECK_THROW(cudaMemcpy(m_dist_buffer, render_buffer.dist_buffer, n_pixels * desired_value * sizeof(float), cudaMemcpyDeviceToHost));
					CUDA_CHECK_THROW(cudaMemcpy(m_density_buffer, render_buffer.density_buffer, n_pixels * desired_value * sizeof(float), cudaMemcpyDeviceToHost));
					CUDA_CHECK_THROW(cudaMemcpy(m_indices_buffer, render_buffer.indices_buffer, n_pixels * desired_value_for_idx * sizeof(uint32_t), cudaMemcpyDeviceToHost));

					float32_t spp_ = (float32_t)spp;						//! uint32_t = unsigned int
					float32_t resolution_x = (float32_t)resolution.x;		//! size_t = long unsigned int
					float32_t resolution_y = (float32_t)resolution.y;		//! size_t = long unsigned int
					float32_t cnt_ = (float32_t)cnt;

					//! 1. spp, resolution_x, resolution_y
					ofs.write((char*)&spp_, sizeof(float32_t));
					ofs.write((char*)&resolution_x, sizeof(float32_t));
					ofs.write((char*)&resolution_y, sizeof(float32_t));
					ofs.write((char*)&cnt_, sizeof(float32_t));
					//TODO: 다음 것도 저장하기 -> desired_value, desired_value_for_idx

					//! 2. n_alive_v, n_steps_between_compaction_v
					for(uint32_t i=0 ; i < n_alive_v.size(); i++)
					{
						float32_t _n_alive_v = (float32_t)n_alive_v[i];
						ofs.write((char*)&_n_alive_v, sizeof(float32_t));
					}
					for(uint32_t i=0 ; i < n_steps_between_compaction_v.size(); i++)
					{
						float32_t _n_steps_between_compaction_v = (float32_t)n_steps_between_compaction_v[i];
						ofs.write((char*)&_n_steps_between_compaction_v, sizeof(float32_t));
					}

					//! 3. Indices (uint32_t를 -> float32_t로 변경 )
					//* 이게 안된다면 다시 수정
					for(uint32_t i = 0 ; i < n_pixels * desired_value_for_idx; i++){
						float32_t tmp = (float32_t)m_indices_buffer[i];
						ofs.write((char*)&(tmp), sizeof(float32_t));
					}

					//! 4. Positions, Directions, RGBAs and dist
					//TODO 값 하나씩 받아와보기 (위에 spp, resolution_x, resolution_y는 잘 됨)
					//* i + j * n_elements
					for (uint32_t i = 0; i < n_pixels * desired_value; i++){
						ofs.write((char*)&(m_position_buffer[i].first), sizeof(float32_t));
						ofs.write((char*)&(m_position_buffer[i].second), sizeof(float32_t));
						ofs.write((char*)&(m_position_buffer[i].third), sizeof(float32_t));
					}

					for (uint32_t i = 0; i < n_pixels * desired_value; i++){
						ofs.write((char*)&(m_direction_buffer[i].first), sizeof(float32_t));
						ofs.write((char*)&(m_direction_buffer[i].second), sizeof(float32_t));
						ofs.write((char*)&(m_direction_buffer[i].third), sizeof(float32_t));
					}

					for (uint32_t i = 0; i < n_pixels * desired_value; i++){
						ofs.write((char*)&(m_rgb_buffer[i].first), sizeof(float32_t));
						ofs.write((char*)&(m_rgb_buffer[i].second), sizeof(float32_t));
						ofs.write((char*)&(m_rgb_buffer[i].third), sizeof(float32_t));
					}

					for (uint32_t i = 0; i < n_pixels * desired_value; i++){
						ofs.write((char*)&(m_density_buffer[i]), sizeof(float32_t));
					}

					for (uint32_t i = 0; i < n_pixels * desired_value; i++){
						ofs.write((char*)&(m_dist_buffer[i]), sizeof(float32_t));
					}

					//! 배열의 동적해제
					delete [] m_position_buffer;
					delete [] m_direction_buffer;
					delete [] m_rgb_buffer;
					delete [] m_dist_buffer;
					delete [] m_density_buffer;
					delete [] m_indices_buffer;

					//! 한 번 저장하는데 약 2~3분 걸림 (64 spp 기준)
					tlog::success() << "The 3D points(" << filename.c_str() << "/" << m_nerf.training.dataset.n_images << ") is saved successfully!!";
					next_training_view();
					++m_cnt;

					if(m_cnt == m_nerf.training.dataset.n_images)
					{
						tlog::success() << "All files are saved successfully!";
						m_cnt = 0;
						m_save_files = !m_save_files;
					}
			...
			if(m_save_json_file){
				//! 광자값들 가져와서 저장하기
				tlog::info() << "Start saving a file";
				std::ofstream ofs(m_camera_path.render_settings.file_path_to_save.c_str(), std::ios::binary);
				...
				//! 한 번 저장하는데 약 2~3분 걸림 (64 spp 기준)
				tlog::success() << "The 3D points(" << m_camera_path.render_settings.file_path_to_save.c_str() << ") is saved successfully!!";
				m_save_json_file = !m_save_json_file;
			}
	...
	// 		if(m_save_json_file){
	// 			//! 광자값들 가져와서 저장하기
	// 			tlog::info() << "Start saving model";
	// 			std::ofstream ofs(m_camera_path.render_settings.file_path_to_save.c_str(), std::ios::binary);

	// 			vec4* rgba = new vec4[n_pixels];
	// 			NerfPayload* tmp = new NerfPayload[n_pixels];
	// 			CUDA_CHECK_THROW(cudaMemcpy(rgba, rays_hit.rgba, n_pixels*sizeof(vec4), cudaMemcpyDeviceToHost));
	// 			CUDA_CHECK_THROW(cudaMemcpy(tmp, rays_hit.payload, n_pixels*sizeof(NerfPayload), cudaMemcpyDeviceToHost));

	// 			unsigned int spp_ = spp;						//! uint32_t = unsigned int
	// 			unsigned int resolution_x = resolution.x;		//! size_t = long unsigned int
	// 			unsigned int resolution_y = resolution.y;		//! size_t = long unsigned int

	// 			ofs.write((char*)&spp_, sizeof(unsigned int));
	// 			ofs.write((char*)&resolution_x, sizeof(unsigned int));
	// 			ofs.write((char*)&resolution_y, sizeof(unsigned int));

	// 			for (uint32_t i = 0; i < n_pixels; i++){
	// 				ofs.write((char*)&(rgba[i].r), sizeof(float32_t));
	// 				ofs.write((char*)&(rgba[i].g), sizeof(float32_t));
	// 				ofs.write((char*)&(rgba[i].b), sizeof(float32_t));
	// 				ofs.write((char*)&(rgba[i].a), sizeof(float32_t));
	// 				ofs.write((char*)&(tmp[i].idx), sizeof(unsigned int));
	// 			}
