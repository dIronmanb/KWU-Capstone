...
void CudaRenderBufferView::clear(cudaStream_t stream) const {
	...
	// printf("(Before)frame_buffer pointer:%x\ndepth_buffer pointer:%x\n", frame_buffer, depth_buffer);
	//* 어디선가 할당이 되어있다!
	...
	CUDA_CHECK_THROW(cudaMemsetAsync(position_buffer, 0, (desired_value) * n_pixels * sizeof(Triple), stream));
	CUDA_CHECK_THROW(cudaMemsetAsync(direction_buffer, 0, (desired_value) * n_pixels * sizeof(Triple), stream));
	CUDA_CHECK_THROW(cudaMemsetAsync(rgb_buffer, 0, (desired_value) * n_pixels * sizeof(Triple), stream));
	CUDA_CHECK_THROW(cudaMemsetAsync(density_buffer, 0, (desired_value) * n_pixels * sizeof(float), stream));
	CUDA_CHECK_THROW(cudaMemsetAsync(dist_buffer, 0, (desired_value) * n_pixels * sizeof(float), stream));
	CUDA_CHECK_THROW(cudaMemsetAsync(indices_buffer, 0, (desired_value_for_idx) * n_pixels * sizeof(uint32_t), stream));
	// printf("(After)coords_buffer pointer:%x\n\n", coords_buffer);
...
void CudaRenderBuffer::resize(const ivec2& res) {
	m_in_resolution = res;
	//! 내가 수정한 부분 01 //
	// m_coords_buffer.enlarge(res.x * res.y); //! ------ !!!! //
	m_position_buffer.enlarge(res.x * res.y * (m_desired_value));
	m_direction_buffer.enlarge(res.x * res.y * (m_desired_value));
	m_rgb_buffer.enlarge(res.x * res.y * (m_desired_value));
	m_dist_buffer.enlarge(res.x * res.y * (m_desired_value));
	m_density_buffer.enlarge(res.x * res.y * (m_desired_value));
	m_indices_buffer.enlarge(res.x * res.y * (m_desired_value)); //TODO 나중에 다시 알아보기
